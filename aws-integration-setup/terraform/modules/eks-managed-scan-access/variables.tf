...
variable "cluster_arns" {
  type        = list(string)
  description = "ARNs of the EKS clusters Nullify scans, all in this module's AWS provider region. AWS provider v5 has no per-resource region, so instantiate the module once per region with providers = { aws = aws.<region> }. The clusters must already exist and their ARNs must be known at plan time: they drive for_each"
