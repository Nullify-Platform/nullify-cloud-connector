...
variable "eks_cluster_arns" {
  type        = list(string)
  description = "ARNs of the EKS clusters whose collector service account the role trusts. Each cluster is looked up through this module's AWS provider, so it must be in the provider's region unless eks_oidc_issuer_urls is set"
  default     = []
...
variable "eks_oidc_issuer_urls" {
  type        = list(string)
  description = "OIDC issuer URLs of the clusters in eks_cluster_arns, in the same order (data.aws_eks_cluster.<name>.identity[0].oidc[0].issuer). AWS provider v5 can only look a cluster up in the provider's region, so set this when any cluster is in another region. Leave empty to look every cluster up in the provider's region"
  default     = []

  validation {
    condition     = alltrue([for url in var.eks_oidc_issuer_urls : can(regex("^https://oidc\\.eks\\.[a-z0-9-]+\\.amazonaws\\.com/id/[A-Za-z0-9]+$", url))])
    error_message = "Each entry must be an EKS OIDC issuer URL: https://oidc.eks.<region>.amazonaws.com/id/<id>"
  }
}
...
variable "kms_key_arn" {
  type        = string
  description = "The KMS ARN shown on the Nullify configure page (optional): a key ARN, including multi-Region key/mrk-... keys, or an alias ARN. A key ARN is preferred because it avoids the wildcard. IAM does not resolve an alias in a policy Resource, so an alias ARN also grants key/* in that same account and region. A concrete key ARN grants that ARN only and does not open every key in Nullify's account. Nullify's key policy is the real gate on a key Nullify owns. The kms_policy_resources output lists what was granted"
  default     = ""
