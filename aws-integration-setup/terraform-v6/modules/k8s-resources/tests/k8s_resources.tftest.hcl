...
run "managed_scan_rbac_only" {
  ...
  assert {
    condition     = tolist(kubernetes_cluster_role_v1.nullify_readonly[0].rule[3].api_groups) == tolist(["networking.k8s.io"]) && toset(kubernetes_cluster_role_v1.nullify_readonly[0].rule[3].resources) == toset(["ingresses", "networkpolicies"])
    error_message = "Rule 3 must grant exactly the networking.k8s.io API group kinds the scanner lists"
  }

  assert {
    condition     = tolist(kubernetes_cluster_role_v1.nullify_readonly[0].rule[4].api_groups) == tolist(["discovery.k8s.io"]) && toset(kubernetes_cluster_role_v1.nullify_readonly[0].rule[4].resources) == toset(["endpointslices"])
    error_message = "Rule 4 must grant exactly the discovery.k8s.io API group kinds the scanner lists"
  }

  assert {
    condition     = tolist(kubernetes_cluster_role_v1.nullify_readonly[0].rule[5].api_groups) == tolist(["rbac.authorization.k8s.io"]) && toset(kubernetes_cluster_role_v1.nullify_readonly[0].rule[5].resources) == toset(["roles", "rolebindings", "clusterroles", "clusterrolebindings"])
    error_message = "Rule 5 must grant exactly the rbac.authorization.k8s.io API group kinds the scanner lists"
  }

  assert {
    condition     = tolist(kubernetes_cluster_role_v1.nullify_readonly[0].rule[6].api_groups) == tolist(["admissionregistration.k8s.io"]) && toset(kubernetes_cluster_role_v1.nullify_readonly[0].rule[6].resources) == toset(["validatingwebhookconfigurations", "mutatingwebhookconfigurations", "validatingadmissionpolicies", "validatingadmissionpolicybindings"])
    error_message = "Rule 6 must grant exactly the admissionregistration.k8s.io API group kinds the scanner lists"
  }
